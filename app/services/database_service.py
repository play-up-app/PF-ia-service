...
class DatabaseService():
    ...
    def saveMatches(self, 
        ...
        try:
            ...
            roundRobinMatches = self._extractRoundRobinMatches(planningId, aiPlanningData, teamsMapping)
            allMatches.extend(roundRobinMatches)

            poulesMatches = self._extractPoulesMatches(planningId, aiPlanningData, teamsMapping)
            allMatches.extend(poulesMatches)

            eliminationMatches = self._extractEliminationMatches(planningId, aiPlanningData, teamsMapping)
    ...
    def _extractRoundRobinMatches(self, 
                                  planningId: str, 
                                  aiPlanningData: AIPlanningData,
                                  teamsMapping: dict) -> List[AIGeneratedMatch]:
        """
    ...
    def _createMatchWithResolvedTeams(self, 
                                planningId: str,
                                match: Match, 
                                phase: str,
                                teamsMapping: dict,
                                poule_id: Optional[str] = None,
                                journee: Optional[int] = None) -> Optional[AIGeneratedMatch]:
        """
        ...
        try:
            ...
            if not resolved_a and not any(ph in match.equipe_a for ph in ['winner_', 'loser_', '1er_', '2e_']):
                print(f"⚠️ Équipe A non trouvée: {match.equipe_a}")
            if not resolved_b and not any(ph in match.equipe_b for ph in ['winner_', 'loser_', '1er_', '2e_']):
    ...
    def _getTeamsMapping(self, planningId) -> dict:
        try:
            ...
            tournamentResult = self.supabase.table("ai_tournament_planning").select("tournament_id").eq("id", planningId).single().execute()
            tournamentId = tournamentResult.data["tournament_id"]

            # recuperer teams grace à tournamentId
            teamsResult = self.supabase.table("team").select("*").eq("tournament_id", tournamentId).execute()
